...
- **Classroom Bulk Actions:** From a classroom roster, select multiple students to create grouped bonuses, penalties, or punishments without reselecting the classroom; all three submissions use classroom-scoped bulk endpoints, and punishment due-date suggestions reuse the current classroom next lessons.
...
- **Student Profile Navigation:** Inline per-section pagination (`< page/total >`) for punishments, bonuses, penalties, and history (profile requests use `item_per_page=5`, respect section page query params like `punishments_page` and `history_page`, default the bonus/punishment panels to `all`, persist the selected bonus/punishment panel filters in dedicated query params such as `bonuses_state`, `punishments_state`, and `punishments_overdue`, and automatically fallback to the previous section page after consume/resolve when the current page becomes empty).
- **Dashboard Section Queries:** Dashboard KPIs still come from `/dashboard`, but the penalties / bonuses / punishments cards now fetch their own paginated `/penalties`, `/bonuses`, and `/punishments` lists with `item_per_page=5`, reuse the student-style section page query params (`penalties_page`, `bonuses_page`, `punishments_page`), reset those section pages back to the first page when the classroom changes, and expose only API-backed panel filters (`all|unused|used` for bonuses, `all|pending|overdue|resolved` for punishments).
...
- `app/components/shared`: Reusable cross-feature building blocks (for example `SectionHeaderPagination`, `BonusesSection`, `PenaltiesSection`, `PunishmentsSection`, and `LoadingButton` for pending form actions).
- `app/components/ui`: Reusable UI primitives (Shadcn).
- `app/composables/services`: API communication layer. **All API calls must go through services.**
- `app/composables`: Reusable business logic and state orchestration, including `useApiActionState` to scope loading states to real API calls after frontend validation passes, shared section composables reused by both dashboard and student-profile adapters, and page-level API orchestration kept out of `app/pages` when the flow carries its own reactive state.
